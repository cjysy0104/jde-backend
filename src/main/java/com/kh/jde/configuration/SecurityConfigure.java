	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
		...
		return httpSecurity.formLogin(AbstractHttpConfigurer::disable)
						   ...
						   .authorizeHttpRequests(requests -> {
							   ...
							   requests.requestMatchers(HttpMethod.DELETE, "/api/members", "/api/reviews/**", "/api/comments/**").authenticated(); // 삭제, 인증 필요한 기능
							   requests.requestMatchers("/api/admin/**", "/api/reports/review/**", "/api/reports/comment/**").hasRole("ADMIN");
							   requests.requestMatchers(HttpMethod.POST, "/api/reviews", "/api/comments", "/api/reports/**", "/api/auth/logout").authenticated(); // 게시글 작성 및 신고 시 로그인 필요
							   requests.requestMatchers(HttpMethod.GET, "/api/reviews/**", "/api/comments/**", "/api/uploads/**", "/api/members/captains").permitAll(); // 게시글 전체조회 및 상세조회는 아무나
